...
# Rain gauge data
I emptied the rain guage in the morning, so it's actually the previous day's rain.
```{r}
rain_gauge <-
...
CAAS_precip <- 
  CAASweather.tidy %>%
  #I'm not actually sure if this is what the "0.1mm" means
  mutate(precip_caas = precip_caas_0.1mm*10) %>%  
  select(date, precip_caas)
