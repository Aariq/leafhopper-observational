...
Later, I asked for weather form may 2017 and asked Hanna to enter it.  Unfortunately there are NAs for May 24th and 25th.  I'm not sure how to deal with that.

```{r}
library(lubridate)
mayweather <- read_csv(here("data", "shaxian climate may 2017.csv"))
mayweather %>% 
  ...
  summarize(temp_mean_caas = mean(Temperature),
            ...
            precip_caas_mm = sum(Precipitation))
```
...
CAASweather.tidy <-
  ...
  rename(
    ...
    precip_caas_mm = rainfall_0_1_mm
  ) %>% 
  mutate(date = as_date(date))
# anyNA(CAASweather.tidy)
```
...
I measured and emptied the rain guage in the morning, so it's actually the previous day's rain.
...
So, I only have HOBO temp data starting the first day of the experiment and only have rain gauge data from a small part of the experiment.  I'd like to fill in the gaps with other data sources so I can use lagged data variables.  But I want to check here if that's a legit thing to do.


## Can I use CAAS precip data?

```{r}
CAAS_precip <- 
  CAASweather.tidy %>%
  select(date, precip_caas_mm)

precip_compare <- full_join(
  rain_gauge,
  CAAS_precip
) 

precip_compare %>% 
  pivot_longer(-date, names_to = "source", values_to = "precip") %>% 
...
Actually, now that I've correctly lagged the rain guage data, it seems kinda close (although not sure if I should multiply by 10 or not.)

```{r}
test <- precip_compare %>% 
  filter(!is.na(precip_mm))
cor.test(test$precip_mm, test$precip_caas_mm)
```

OK, that's good.  I'll use it!  But becasue the scale seems kinda different, I should just use only the CAAS numbers. I think.

## Can I use CAAS temperature data?

```{r}
full_join(daily.temps, CAASweather.tidy) %>%
  arrange(date) %>% 
  filter(year(date) == 2017) %>% 
  ggplot(aes(x = date)) +
  geom_line(aes(y = temp_mean_caas), color = "blue") +
  geom_point(aes(y = temp_mean_caas), color = "blue") +
  geom_line(aes(y = temp_mean), color = "red") +
  geom_point(aes(y = temp_mean), color = "red")
```

Oh, yeah, this is fine.  No correlation test needed. But unfortunately it's only 2 days of extra data.
