...
```{r}
shoot_data_joined <-
  left_join(shoot.data, temperature, by = "date") %>% 
...
shoot_data_full <-
  shoot_data_joined %>%
  group_by(shoot_id) %>% 
  group_modify(~bind_rows(., prevtemp)) %>% 
...
shootdf <- shoot.data7 %>%
  select(date, shoot_id, interharvest_id, growth, diameter, day_post, mean_day, starts_with("temp"), starts_with("precip"))
```
