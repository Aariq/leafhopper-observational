...
temp_lags <- map_dfc(1:7, ~lag(weather$temp_mean, .x)) %>% set_names(glue::glue("temp_lag{1:7}"))
precip_lags <- map_dfc(1:7, ~lag(weather$precip_mm, .x)) %>% set_names(glue::glue("precip_lag{1:7}"))
