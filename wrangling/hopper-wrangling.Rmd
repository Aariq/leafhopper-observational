...
Join temperature summary data.
```{r join with temperature data}
#because leafhopper counts are early in the morning, the weather the previous day is most relevant.  Create an "effective date" and merge by that
daily.temps <- 
  daily.temps %>% 
  mutate(effective.date = Date + days(1))

hopper.data.full <- inner_join(hopper.data, daily.temps, by = c("Date" = "effective.date")) %>%
  rename(date.temp = Date.y)
hopper.data.full
```
...
hourly <- temp.data %>%
  dplyr::select(-Date) %>% 
  mutate(rounded = round_date(date_time, unit = "hour")) #create a column of times rounded to the nearest hour

hopper.data.full2 <- hopper.data.full %>% 
  mutate(rounded = round_date(Count.Dtime, unit = "hour"))

#join and clean up columns used only for joining
hopper.data.full <- left_join(hopper.data.full2, hourly, by = "rounded") %>% 
  dplyr::select(-rounded, - Count.Dtime) %>% 
  rename(count.temp = temp_c)
...
```{r save hopper data}
write_rds(hopper.data.full, "hopper data.rds")
```
## Shoot Data
