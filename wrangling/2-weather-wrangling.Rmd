...
weather_lags <- 
  bind_cols(weather, temp_lags, temp_min_lags, temp_max_lags, precip_lags, L) %>% 
  select(date, starts_with("Q_"), starts_with ("L"))
```

Convert to matrix columns for convenience in modeling

```{r paged.print=FALSE}
weather_lags <- 
  weather_lags %>% 
  mutate(
    Q_temp_mean = as.matrix(select(., starts_with("lag_temp_mean"))),
    Q_temp_min = as.matrix(select(., starts_with("lag_temp_min"))),
    Q_temp_max = as.matrix(select(., starts_with("lag_temp_max"))),
    Q_precip = as.matrix(select(., starts_with("lag_precip"))),
    L = as.matrix(select(., starts_with("L_"))),
    ) %>% 
  select(
    -starts_with("lag_temp_mean"), 
    -starts_with("lag_temp_min"),
    -starts_with("lag_temp_max"),
    -starts_with("lag_precip"),
    -starts_with("L_")
  )

weather_lags <- full_join(weather, weather_lags)
```
