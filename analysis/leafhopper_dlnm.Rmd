...
hopper_temp_raster <-
  ...
  geom_contour(color = "grey50", size = 0.4, binwidth = 0.001) +
...
weather <- weather %>% 
  filter(date > min(hoppers$date) - days(15),
...
hopper_precip_raster +
  ...
  plot_annotation(tag_levels = "A") &
  theme_classic()
ggsave(here("figs", "hopper_crossbasis_rug.png"), shoot_cb, width = 165, units = "mm")
```
