...
hoppers <-
  ...
  mutate(pick_temp_mean = mean(Q_temp_mean[ , 1:12]),
         pick_temp_min = mean(Q_temp_min[ , 1:12]),
         pick_temp_max = mean(Q_temp_max[ , 1:12]),
         pick_precip = mean(Q_temp_max[ , 1:12])) %>% 
  ungroup()
```
...
h_temp_max_h2 <- anova.gam(m_hopper_temp_max_null, m_hopper_temp_max,
                           test = "Chisq")
h_temp_max_h2
#larger residual DF means smaller estimated degrees of freedom
```
