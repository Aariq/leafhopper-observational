...
ggplot(hoppers, aes(x = date, y = temp_mean)) +
...
bbmle::ICtab(m_hopper_temp, m_hopper_precip, m_hopper_date)
```
...
hoppers2 <- hoppers %>% 
  group_by(plant) %>% 
  mutate(hoppers_prev = lag(hoppers),
         n_leaves_prev = lag(n_leaves)) %>% 
  filter(!is.na(n_leaves_prev)) %>% 
  ungroup() %>% 
  mutate(h_per_l = hoppers / n_leaves,
         h_per_l_prev = hoppers_prev / n_leaves_prev)


ggplot(hoppers2, aes(h_per_l - h_per_l_prev)) + geom_histogram()
...
```{r}
m_rate <- 
  gam(h_per_l - h_per_l_prev ~ #change in hoppers per leaf from the previous day
        # s(Q_precip2, L2,
        #   bs = "cb",
        #   k = c(5, 7),
        #   xt = list(bs = "cr")
