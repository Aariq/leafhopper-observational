...
```{r}
plot_gam_check(m_hopper_min_temp)
#check for overdispersion
...
# Maximum temperature
```{r}
plot_gam_check(m_hopper_max_temp)
#check for overdispersion
...
```{r}
d <- list("Mean Temperature" = m_hopper_temp,
         "Minimum Temperature" = m_hopper_min_temp,
...
`visibly` pacakge can't handle these cross-basis smooths apparently.

I suspect that `pred_hoppers()` isn't working correctly now that I'm specifying the lag matrix explicitly.

```{r}
ht_pd <-
  pred_hoppers(Q_temp, lag = 15, hoppers, "temp_mean", m_hopper_temp)

htmin_pd <- 
  pred_hoppers(Q_min_temp, lag = 15, hoppers, "temp_min", m_hopper_min_temp)

htmax_pd <- 
  pred_hoppers(Q_max_temp, lag = 15, hoppers, "temp_max", m_hopper_max_temp)

hp_pd <-
  pred_hoppers(Q_precip, lag = 15, hoppers, "precip", m_hopper_precip)
```
