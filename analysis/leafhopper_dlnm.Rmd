...
Let's see if I can fit a model that estimates the growth rate of leafhoppers per leaf.  Then I can just ignore the "day" covariate and maybe even the days_post covariate.


$$
log \bigg(\frac{H_t/L_t}{H_{t-1}/L_{t-1}} \bigg) = \beta_0 + ...
$$
$$
 log\bigg(\frac{H_t \times L_{t-1}}{H_{t-1} \times L_t}\bigg) = log(H_t) + log(L_{t-1}) - log(H_{t-1}) -log(L_t)
$$
$$
log(H_t) = \beta_0 +...+ \textrm{offset}[log(L_t) + log(H_{t-1}) - log(L_{t-1})]
$$
...
hoppers2 <- hoppers %>% 
  group_by(plant) %>% 
  mutate(hoppers_next = lead(hoppers),
         n_leaves_next = lead(n_leaves)) %>% 
  filter(!is.na(n_leaves_next)) %>% 
  ungroup() %>% 
  mutate(h_per_l = hoppers / n_leaves,
         h_per_l_next = hoppers_next / n_leaves_next)


# ggplot(hoppers2, aes(h_per_l_next - h_per_l)) + geom_histogram()
...
L2 <- matrix(0:(ncol(Q_temp2)-1), nrow(Q_temp2), ncol(Q_temp2), byrow = TRUE)
```
...
m_offsets_temp <- 
  gam(hoppers ~
        ...
        s(plant, bs = "re"),
      family = poisson(),
      offset = log(n_leaves) + log(hoppers_prev) - log(n_leaves_prev),
      data = hoppers2,
      method = "REML")
beepr::beep(3)
```

Ah, nevermind.  Can't do this because there are a lot of zeroes in the data.  Can't calculate a rate like this.  buhhh..

But I *can* model algebraic growth rates as a function of temperature, etc.

```{r}
m_rate <- 
  gam(h_per_l_next - h_per_l ~ #change in hoppers per leaf from the previous day
        s(Q_precip2, L2,
          bs = "cb",
          k = c(5, 7),
          xt = list(bs = "cr")
...
The growth *rate* doesn't seem to depend on anything (other than who counted leafhoppers *sigh*).
...
Q_temp_new <- matrix(mean(hoppers2$temp_mean), nrow = length(testvals), ncol = ncol(Q_temp))
...
hopper_temp_contour <-
  ggplot(ht_pd, aes(x = x, y = lag, z = fitted, fill = fitted)) +
  geom_contour_filled(aes(fill = stat(level)), breaks = pretty(seq(0.0348, 0.0659, 0.0001), n = 12)) + 
...
Q_precip_new <- matrix(mean(hoppers2$precip_mm), nrow = length(testvals), ncol = ncol(Q_precip))
