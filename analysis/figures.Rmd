...
hopper_lim <- c(min(exp(ht_pd$fitted), 
                ...
                max(exp(ht_pd$fitted),
                    ...
                    exp(htmax_pd$fitted),
                    exp(hp_pd$fitted)))
contour_binwidth <- 0.002 #distance between contour lines (in leafhoppers/leaf)
```
...
hopper_temp_raster <-
  ...
  geom_tile(data = filter(ht_pd, min_dist >= 0.1), color = "grey50", fill = "grey50", alpha = 0.4) +
...
hopper_min_temp_raster <-
  ...
  geom_raster(interpolate = TRUE) +
  geom_contour(color = "grey50", size = 0.4, binwidth = contour_binwidth) +
  geom_tile(data = filter(htmin_pd, min_dist >= 0.1), color = "grey50", fill = "grey50", alpha = 0.4) +
  scale_fill_viridis_c("Leafhoppers\nper leaf",
                       option = "A", 
                       limits = hopper_lim) + 
  scale_y_continuous("Lag (days)", expand = c(0,0)) + 
...
#11 days lag
```
11 days, if you don't count the hatched area.
...
hopper_max_temp_raster <-
  ...
  geom_raster(interpolate = TRUE) +
  geom_contour(color = "grey50", size = 0.4, binwidth = contour_binwidth) +
  geom_tile(data = filter(htmax_pd, min_dist >= 0.1), color = "grey50", fill = "grey50", alpha = 0.4) +
  scale_fill_viridis_c("Leafhoppers\nper leaf",
                       option = "A",
                       limits = hopper_lim) + 
  scale_y_continuous("Lag (days)", expand = c(0,0)) + 
...
hopper_precip_raster <-
  ...
  geom_raster(interpolate = TRUE) +
  geom_contour(color = "grey50", size = 0.4, binwidth = contour_binwidth) +
  geom_tile(data = filter(hp_pd, min_dist >= 0.1), color = "grey50", fill = "grey50", alpha = 0.4) +
  scale_fill_viridis_c("Leafhoppers\nper leaf",
                       option = "A",
                       limits = hopper_lim) + 
  scale_y_continuous("Lag (days)", expand = c(0,0)) + 
...
hopper_min_temp_raster +
  geom_rug(data = weather, inherit.aes = FALSE, aes(x = temp_min), color = "grey50") +
hopper_temp_raster +
  geom_rug(data = weather, inherit.aes = FALSE, aes(x = temp_mean), color = "grey50") +
hopper_max_temp_raster +
  geom_rug(data = weather, inherit.aes = FALSE, aes(x = temp_max), color = "grey50") +
hopper_precip_raster +
  geom_rug(data = weather, inherit.aes = FALSE, aes(x = precip), color = "grey50") +
  plot_layout(guides = "collect") +
  plot_annotation(tag_levels = "A") &
  theme_classic() +
  theme(text = element_text(size = 10))
ggsave(here("figs", "hopper_crossbasis_rug.png"), width = 165, units = "mm")
```
...
growth_lims <- c(min(st_pd$fitted, 
                 ...
                 max(st_pd$fitted,
                     ...
                     stmin_pd$fitted,
                     stmax_pd$fitted))
contour_binwidth <- 0.01 #distance between contour lines (in cm)
```
...
shoot_temp_raster <-
  ...
  geom_raster(interpolate = TRUE) +
  geom_contour(color = "grey50", size = 0.4, binwidth = contour_binwidth) +
  geom_tile(data = filter(st_pd, min_dist >= 0.1), color = "grey50", fill = "grey50", alpha = 0.4) +
...
shoot_min_temp_raster <-
  ...
  geom_raster(interpolate = TRUE) +
  geom_contour(color = "grey50", size = 0.4, binwidth = contour_binwidth) +
  geom_tile(data = filter(stmin_pd, min_dist >= 0.1), color = "grey50", fill = "grey50", alpha = 0.4) +
  scale_fill_viridis_c("Shoot growth\n(cm/day)", option = "A", limits = growth_lims) + 
  scale_y_continuous("Lag (days)", expand = c(0,0)) + 
  scale_x_continuous("Daily Minimum Temperature (ºC)", expand = c(0,0)) 
shoot_min_temp_raster
...
shoot_max_temp_raster <-
  ...
  geom_raster(interpolate = TRUE) +
  geom_contour(color = "grey50", size = 0.4, binwidth = contour_binwidth) +
  geom_tile(data = filter(stmax_pd, min_dist >= 0.1), color = "grey50", fill = "grey50", alpha = 0.4) +
  scale_fill_viridis_c("Shoot growth\n(cm/day)", option = "A", limits = growth_lims) + 
  scale_y_continuous("Lag (days)", expand = c(0,0)) + 
  scale_x_continuous("Daily Maximum Temperature (ºC)", expand = c(0,0))
shoot_max_temp_raster
...
shoot_precip_raster <-
  ...
  geom_raster(interpolate = TRUE) +
  geom_contour(color = "grey50", size = 0.4, binwidth = contour_binwidth) +
  geom_tile(data = filter(sp_pd, min_dist >= 0.1), color = "grey50", fill = "grey50", alpha = 0.4) +
  scale_fill_viridis_c("Shoot growth\n(cm/day)", option = "A", limits = growth_lims) + 
  scale_y_continuous("Lag (days)", expand = c(0,0)) + 
  scale_x_continuous("Daily Precipitation (mm)", expand = c(0,0))
shoot_precip_raster
...
shoot_precip_raster +
  ...
  theme_classic() +
  theme(text = element_text(size = 10))
ggsave(here("figs", "shoot_crossbasis_rug.png"), width = 165, units = "mm")
```
