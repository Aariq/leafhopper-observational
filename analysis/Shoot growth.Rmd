...
ee <- Effect("temp_lag1", temp.lmer3) 
...
```{r}
precip_lmer <-
  lmer(growth ~ precip_lag1 + I(precip_lag1^2) + day_post + diameter + mean_day + (1|interharvest_id),
                   data = precipdf)
```
...
```{r}
precip_lm <-
  lm(growth ~ precip_lag1 + I(precip_lag1^2) + day_post + diameter + mean_day,
                   data = precipdf)

ICtab(precip_lmer, precip_lm)
```
Ok, for precipitation the random effect doesn't matter.  I'll go with the linear model I guess.

## Does mean_day matter?
Is there an effect of time in the season?

```{r}
precip_lm2 <-
  lm(growth ~ precip_lag1 + I(precip_lag1^2) + day_post + diameter,
                   data = precipdf)
ICtab(precip_lm, precip_lm2)
```
Yes, time in the season does matter.

## Linear or quadratic effect of precip?

```{r}
precip_lm3 <- 
    lm(growth ~ precip_lag1 + day_post + diameter +  mean_day,
                   data = precipdf)
ICtab(precip_lm, precip_lm3)
```
Linear fits equally well.
...
- Increased precipitation significantly but slightly decreases growth (through shading?)
- fatter stems grow faster
- growth slows after harvest
- growth slows as the summer progresses
...
```{r}
precip_lag_lmer <-
  lmer(growth ~ precip_k + day_post + diameter + mean_day + (1|interharvest_id),
                   data = precipdf)
```
...
```{r}
precip_lag_lm <-
  lm(growth ~ precip_k + day_post + diameter + mean_day,
                   data = precipdf)
...
```{r}
precip_lag_lm3 <- 
    lm(growth ~ precip_k_lin + day_post + diameter +  mean_day,
                   data = precipdf)
ICtab(precip_lag_lm, precip_lag_lm3)
```
...
plot(precip_lag_lm)
plot(precip_lm3)
hist(precipdf$growth)

# library(predictmeans)
# anovalmer(temp.lmer3)
```
