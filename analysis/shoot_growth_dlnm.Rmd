...
dph_plot <-
  ...
  # geom_point(data = shootdf, aes(y = growth, x = day_post), inherit.aes = FALSE, alpha = 0.05) +
  geom_line(size = 1) +
...
Look at cross-sections to see how increases in climate variance might effect shoot growth. See: Rudgers, Jennifer A., Y. Anny Chung, Gregory E. Maurer, Douglas I. Moore, Esteban H. Muldavin, Marcy E. Litvak, and Scott L. Collins. “Climate Sensitivity Functions and Net Primary Production: A Framework for Incorporating Climate Mean and Variability.” Ecology 99, no. 3 (2018): 576–82. https://doi.org/10.1002/ecy.2136.

## Mean temp

Strongest effect at 1 day of lag.  Concave downward, so increased variance around mean would be detrimental to shoot growth.  Also convex at lag = 14 days, where there also seems to be somethign going on (probably spurrious though).

```{r fig.height=7, fig.width=5}
a <- 
  shoot_temp_raster +
  geom_hline(aes(yintercept = 1), linetype = 1) +
  geom_vline(aes(xintercept = 26.9), linetype = 2)
...
b <- 
  ...
  ylim(growth_lims) +
  theme(text = element_text(size = 12), aspect.ratio = 1) +
  labs(x = "Temperature (ºC)", y = "shoot growth\n(cm/day)")

st_cross <- a/b &
  plot_annotation(title = "Shoot growth vs. mean daily temperature",
                  caption = "Concave relationship means that increase in\n variance around mean is detrimental to growth")
ggsave(here("docs", "st_cross.png"), st_cross)
st_cross
```

## Min temp

Relationship strongest at lag = 1.  Fairly linear response, so shoot growth not very sensitive to increased variation in min temp.

```{r fig.height=7, fig.width=5}
a <- 
  shoot_min_temp_raster +
  geom_hline(aes(yintercept = 1), linetype = 1) +
  geom_vline(aes(xintercept = mean(shootdf$temp_min)), linetype = 2)
...
stmin_cross <- a/b &
  plot_annotation(title = "Shoot growth vs. minimum daily temperature",
                  caption = "Linear relationship means that shoot growth is \n relatively insensitive to increased variation around mean")
stmin_cross
ggsave(here("docs", "stmin_cross.png"), stmin_cross)
```

## Precip

Relationship strongest at 1 day lag.  Concave downward, so increased variance in precip would be detrimental to shoot growth.

```{r fig.height=7, fig.width=5}
a <- 
  shoot_precip_raster +
  geom_hline(aes(yintercept = 1), linetype = 1) +
  geom_vline(aes(xintercept = 6.9), linetype = 2)
...
b <- 
  ...
  ylim(growth_lims) +
  theme(text = element_text(size = 12), aspect.ratio = 1) +
  labs(x = "Precipitation (mm)", y = "shoot growth\n(cm/day)")

sp_cross <- a/b & 
  plot_annotation(title = "Shoot growth vs. precipitation",
                  caption = "Concave relationship means that increase in\n variance around mean is detrimental to growth")
ggsave(here("docs", "sp_cross.png"), sp_cross)
sp_cross
```
