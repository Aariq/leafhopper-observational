...
```{r}
# plot(m_temp, residuals = TRUE, rug = FALSE, ylim = c(-1, 1))
# png(here("figs", "shoot_days_post.png"))
plot(m_temp, select = 2, xlab = "days post harvest", seWithMean = TRUE, shade = TRUE)
# dev.off()

# png(here("figs", "shoot_diameter.png"))
plot.gam(m_temp, select = 3, xlab = "shoot diameter (mm)", seWithMean = TRUE, shade = TRUE)
# dev.off()
```
...
pred_temp <- crosspred("Q_temp", m_temp, cen = cen)
pred_temp2 <- crosspred("Q_temp", m_temp, by = 0.2, bylag = 0.2, cen = cen)
