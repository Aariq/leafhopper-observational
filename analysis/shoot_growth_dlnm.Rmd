...
shoot_temp_raster_magma <- shoot_temp_raster + scale_fill_viridis_c("Shoot growth \n(cm/day)", option = "A")
shoot_temp_contour_magma <- shoot_temp_contour + scale_fill_viridis_d("Shoot growth \n(cm/day)", option = "A", drop = FALSE)

shoot_temp_colortest <-
  shoot_temp_raster + 
  shoot_temp_contour + 
  shoot_temp_raster_magma +
  shoot_temp_contour_magma
save_plot(here("shoot_temp_test.png"), shoot_temp_colortest, nrow = 2, ncol = 2)
```
...
```{r}
# png(here("figs", "shoot_diameter.png"))
plot.gam(m_shoot_precip, seWithMean = TRUE, shift = coef(m_shoot_precip)[1]) -> shoot_pd
# dev.off()

dia <- shoot_pd[[3]]
dia_plot <- 
  tibble(x = dia$x, se = dia$se, y = dia$fit + coef(m_shoot_precip)[1]) %>% 
  ggplot(aes(x, y)) + 
  geom_line() +
  geom_ribbon(aes(ymin = y-se, ymax = y+se), alpha = 0.2) +
  geom_rug(aes(x = raw, y =y), data = tibble(raw = dia$raw, y = 0),
...
dph <- shoot_pd[[2]]
dph_plot <- 
  tibble(x = dph$x, se = dph$se, y = dph$fit + coef(m_shoot_precip)[1]) %>% 
  ggplot(aes(x, y)) + 
  geom_line() +
  geom_ribbon(aes(ymin = y-se, ymax = y+se), alpha = 0.2) +
  geom_rug(aes(x = raw, y =y), data = tibble(raw = dph$raw, y = 0),
           sides = "b", alpha = 0.1) +
  scale_y_continuous("Shoot growth (cm/day)", limits = c(0,0.5)) +
  scale_x_continuous("Days post harvest") +
  theme_bw()
...
resp_p[836, 1, drop = FALSE]

which(resp_p == max(resp_p), arr.ind = TRUE)
resp_p[151, 4, drop = FALSE]
```
...
shoot_precip_raster_magma <- shoot_precip_raster + scale_fill_viridis_c("Shoot growth \n(cm/day)", option = "A")
shoot_precip_contour_magma <- shoot_precip_contour + scale_fill_viridis_d("Shoot growth \n(cm/day)", option = "A", drop = FALSE)
shoot_precip_colortest <-
  shoot_precip_raster + 
  shoot_precip_contour + 
  shoot_precip_raster_magma +
  shoot_precip_contour_magma

save_plot(here("shoot_precip_test.png"), shoot_precip_colortest, nrow = 2, ncol = 2)
```

test of all

```{r}
viridis_pal <- (shoot_precip_raster + shoot_temp_raster + plot_layout(guides = "collect"))
magma_pal <- (shoot_precip_raster_magma + shoot_temp_raster_magma + plot_layout(guides = "collect"))
viridis_pal_c <- (shoot_precip_contour + shoot_temp_contour + plot_layout(guides = "collect"))
magma_pal_c <- (shoot_precip_contour_magma + shoot_temp_contour_magma + plot_layout(guides = "collect"))
colortest <- viridis_pal /magma_pal / viridis_pal_c / magma_pal_c

save_plot(here("colortest.png"), colortest, ncol = 2, nrow = 4)
```
