...
Create matrices for temperature, precip, and lag.  Zero days lag doesn't make sense since measurements were taken in the morning.  Remove those
...
```{r}
#optimum:
which(resp_t == max(resp_t[ , 1, drop = FALSE]), arr.ind = TRUE)
resp_t[129, 1, drop = FALSE]

#min temp:
resp_t[1, 1, drop = FALSE]

#max temp:
resp_t[nrow(resp_t), 1, drop = FALSE]
```
...
```{r}
#max growth
which(resp_p == max(resp_p[ , 1, drop = FALSE]), arr.ind = TRUE)
resp_p[34, 1, drop = FALSE]

#at min precip
resp_p[1, 1, drop = FALSE]

#at max precip
resp_p[nrow(resp_p), 1, drop = FALSE]
```
...
Unclear why precip model covariates are consistently higher, but it might be a mistake.

```{r}
predict.gam(m_shoot_precip, terms = "s(diameter)", type = "terms")[1:10]>
predict.gam(m_shoot_temp, terms = "s(diameter)", type = "terms")[1:10]

predict.gam(m_shoot_precip, terms = "s(day_post)", type = "terms")[1:10]>
predict.gam(m_shoot_temp, terms = "s(day_post)", type = "terms")[1:10]
```
newdata:

```{r}
newdata_temp = list(
          # Q_precip = Q_precip_new,
          Q_temp = Q_temp_new,
          L = L_new,
          day_post = day_post_new,
          diameter = seq(min(shootdf$diameter), max(shootdf$diameter), length.out = length(interharvest_id_new)),
          interharvest_id = interharvest_id_new,
          plant = plant_new)
newdata_precip = list(
          Q_precip = Q_precip_new,
...
predict.gam(m_shoot_precip, newdata = newdata_precip, type = "response")[190:200] ->precip_10
predict.gam(m_shoot_temp, newdata = newdata_temp, type = "response")[190:200] -> temp_10
precip_10>temp_10
```

hmmm.... precip is always larger, but these numbers don't match partial effects plots.  What's a parial effects plot??
...
```{r}
growth_lims <- c(min(resp_p, resp_t), max(resp_p, resp_t))
```
