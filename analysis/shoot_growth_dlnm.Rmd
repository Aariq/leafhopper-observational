...
```{r}
plot_gam_check(m_shoot_min_temp)
```
...
```{r}
plot_gam_check(m_shoot_max_temp)
```
...
```{r}
covar_plot_data <- list("Mean Temperature" = m_shoot_temp,
         "Minimum Temperature" = m_shoot_min_temp,
...
```{r}
st_pd <- cb_margeff(m_shoot_temp, Q_temp, L) 
stmin_pd <- cb_margeff(m_shoot_min_temp, Q_min_temp, L)
stmax_pd <- cb_margeff(m_shoot_max_temp, Q_max_temp, L)
